/*
 * Nutrition interaction fallback for file:// usage.
 * Preferred local run mode for development remains: python3 -m http.server
 */
(function initNutritionFallbackIIFE() {
...
function initNutritionInteraction() {
    ...
    function bootNutritionFallback() {
        if (window.__nutritionFallbackLoaded) return;
        initNutritionInteraction();
        window.__nutritionFallbackLoaded = true;
    }

    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', bootNutritionFallback);
    } else {
        bootNutritionFallback();
    }
}());
